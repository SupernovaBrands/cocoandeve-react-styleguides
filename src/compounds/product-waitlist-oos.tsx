...
const ProductWaitlist = (props: any) => {
	...
	return (
		<>
				...
				{/* <form data-pdp="false" data-product-id="product-id"> */}
					{ !props.hide_phone && <><div className="row">
						<div className="relative flex items-stretch w-full mb-g">
							...
							<input onChange={(e:any) => changePhone(e)} id="waitlist-phone" className="focus:outline-none block border-none appearance-none w-full py-1 px-2 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded border-left-1 rounded-tl-none rounded-bl-none" type="tel" placeholder="Phone number" aria-label="phone" />
						</div>
					...
					<div className="row">
						<div className="relative flex items-stretch w-full flex-grow max-w-full flex-1 mb-1">
							<input onChange={(e:any) => changeEmail(e)} className="focus:outline-none block appearance-none w-full py-1 px-2 text-base leading-normal bg-white text-gray-800 rounded border-left-1" type="email" name="email" placeholder="Enter your email" />
						</div>
						{ emailError && <small className="col-12 text-primary email-error">Please enter a valid email address</small> }
					</div>
                    <div className="flex flex-wrap items-center justify-start">
                    <div className="-mt-1 w-full">
                        <CheckBox onChange={changeTos} labelClass="flex justify-content-center my-1 relative pl-3 font-size-sm mt-2" id="agreement-waitlist" checked={true}>
							<span>I agree to <a className="font-size-sm" href="#">Privacy Policy & ToS</a></span>
						</CheckBox>
					...
					<div className="flex my-1">
						<input onClick={submitForm} disabled={!validForm} className="inline-block align-middle text-center select-none border whitespace-no-wrap  px-3  no-underline  leading-tight text-base bg-blue-600 hover:bg-blue-600 w-full bg-primary text-white py-1 font-bold pt-[13px] pb-[13px] rounded-[8px]" type="button" value="Submit" />
					</div>
			...
			{ showSuccess && <div className="w-100 p-3 mb-3 rounded text-center">
				<h3 className="mx-4 mx-lg-5">{data.titleThanks}</h3>
