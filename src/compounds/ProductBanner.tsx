...
const ProductBanner = (props: any) => {
	...
	return (
        <div className={`product__banner flex mx-0 mb-0 flex-wrap ${props.background} ${props.reverse ? 'flex-row-reverse' : ''} ${props.mainClasses ?? ''}`}>
            <div className={`product__banner-left w-full ${props.textContentClasses ? props.textContentClasses : 'lg:grid-cols-[1fr_repeat(6,_[_col-start_]_minmax(0,_70px))]'} lg:w-1/2 grid gap-x-[30px] content-center`}>
                <div className={`product__banner-content h-auto ${props.textContentBoxClasses ? props.textContentBoxClasses : 'lg:col-start-[col-start_1] lg:col-end-[span_5] mb-0'}`}>
                    {props.children}
                </div>
            </div>
            <div className="product__banner-right w-full lg:w-1/2 px-0 relative">
                {comparisonImages ? (
                    ...
                    <>
                        <picture className={`block ${props.pictureClasses ?? 'pt-[86%]'} w-full overflow-hidden`}>
                            <source
                                srcSet={props.srcSet || props.src}
                                media="(min-width: 992px)" width="1362" height="1162"/>
                            <img
                                src={props.src}
                                className={`${props.imageClasses ?? 'embed-responsive-item object-cover'} h-full w-full`} loading="lazy" height="357" width="414" alt={"Product banner and comparison image"} />
                        </picture>
                    </>
                )}
            </div>
