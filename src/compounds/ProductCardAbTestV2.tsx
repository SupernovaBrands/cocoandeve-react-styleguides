...
const DEFAULT_LABEL = '<span className="lg:text-sm lg:font-semibold">Add</span>';
...
const Pricing = ({ props, collectionTemplate, hideCent, selectedVariant, store, overlay = true }) => {
    ...
    return (
        <>
            <span className={`${collectionTemplate ? 'border-x border-x-transparent' : ''} text-sm product-card-btn__text lg:w-auto flex ${props.carousel || props.collectionTemplate ? 'w-auto text-nowrap text-left py-[.5em]' : props.sideUpsell ? 'w-full lg:w-full text-center text-sm' : 'w-full text-center lg:text-left'} `}>
            ...
            {<span className={`${collectionTemplate ? 'border-x border-x-transparent' : ''} product-card-btn__prices lg:w-auto flex ${props.carousel || props.collectionTemplate ? 'w-auto text-right py-[.5em]' : props.sideUpsell ? 'w-full lg:w-full text-center text-sm' : 'w-full text-center lg:text-right'}`}>
                {props.comparePrice && (<span className="line-through text-sm mr-25 font-normal">
...
const WaitlistButton = (props:any) => {
    ...
    const data = {...props, ...{ label: `<span class="text-sm lg:font-semibold">Waitlist</span>` }};
    return (
        <Button onClick={handleWaitlist} buttonClass={`${props.className ?? ''} w-full text-primary font-semibold text-primary border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} !h-[38px] !min-h-[38px] flex flex-row btn-sm md:text-base bg-gray-400 text-primary rounded-[8px] mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1 justify-between' : 'sm:flex-col sm:text-sm  '}`}>
...
const LaunchButton = (props: any) => {
    ...
    const defaultText = 'Waitlist Me';
    const data = {...props, ...{ label: `<span class="text-sm">Waitlist</span>` }};
    return (
        <Button onClick={handleLaunchWaitlist} buttonClass={`${props.className ?? ''} w-full !h-[38px] !min-h-[38px] border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} flex flex-row btn-sm md:text-base bg-gray-400 text-primary font-semibold rounded-[8px] mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1 justify-between' : 'sm:flex-col sm:text-sm  '}`}>
            <Pricing store={props.store} selectedVariant={props.selectedVariant} collectionTemplate={props.collectionTemplate} props={data} hideCent={true}/>
        </Button>
    )
}
...
const AddToCartButton = (props:any) => {
    ...
    return (
        <Button onClick={onAddItem} buttonClass={`${props.className ?? ''} w-full lg:w-auto !h-[38px] !min-h-[38px] product-card-btn border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} flex flex-row btn-sm md:text-base bg-gray-400 text-primary font-bold text-primary rounded-[8px] mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1' : props.sideUpsell ? 'flex flex-col sm:text-sm lg:flex-col lg:justify-center lg:py-[5px]' : 'sm:flex-col sm:text-sm  '}`}>
...
const SwatchOverlay = (props:any) => {
    ...
    labelText = `<span class="lg:text-sm lg:font-semibold">Add</span>`;
        ...
        <>
                ...
                return resultVariant && (
                    <>
                        ...
                        {resultVariant.availableForSale && (
                            <AddToCartButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} preOrders={preOrders} comparePrice={comparePrice} price={price} selectedVariant={selectedVariant} carousel={props.carousel} addToCart={addToCart} className="button-overlay z-[1] w-full mb-0 lg:flex lg:justify-center" overlay={true}/>
                        )}
                        {!resultVariant.availableForSale && (
                            <WaitlistButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} setWaitlistData={props.setWaitlistData} product={props.product} selectedVariant={resultVariant} comparePrice={comparePrice} price={price} carousel={props.carousel} className="button-overlay z-[1] w-full mb-0 lg:flex lg:justify-center" overlay={true} />
                        )}
                    </>
                )
            })()}
            ...
            {!props.quizResult && (
                <>
                    ...
                    <div className={`!w-auto px-0 swatch-overlay ${props.sideUpsell ? 'left-[5px] lg:left-[5px] right-[5px] lg:right-[5px] bottom-[35px]' : '!left-[0px] !lg:left-[8px] !right-[0px] !lg:right-[8px] bottom-[35px]'} flex-col items-center justify-end pb-0 absolute bg-white lg:px-0 border border-gray-400 rounded-t`}>
                        <div className={`text-center w-full pt-2 lg:pb-2 pb-1 ${props.sideUpsell ? 'lg:px-0' : 'lg:px-1'}`}>
                            ...
                            <ul className="list-unstyled product-variant-swatch flex justify-center">
                                {props.swatch.data.length > 0 && props.swatch.data.map((item:any, i:any) => (
                                    <li key={`swatch-card-${item.id}`} className={`${props.sideUpsell ? 'w-[42px]' : 'w-auto lg:!w-1/3'} product-variant-swatch__item ${item.available ? 'available' : 'oos'} ${selectedVariant.id === item.id ? 'active' : ''}`} data-available={item.available ? 'available': ''}>
                        ...
                        {swatchAvailable && (
                            <AddToCartButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} label={label} preOrders={preOrders} comparePrice={comparePrice} price={price} selectedVariant={selectedVariant} carousel={props.carousel} addToCart={addToCart} className="button-overlay z-[1] w-full mb-0 lg:flex lg:justify-betweeen" sideUpsell={props.sideUpsell} trackEvent={props?.trackEvent || null} overlay={true} />
                        )}
                        {!swatchAvailable && (
                            <WaitlistButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} setWaitlistData={props.setWaitlistData} product={props.product} selectedVariant={selectedVariant} comparePrice={comparePrice} price={price} carousel={props.carousel} className="button-overlay z-[1] w-full mb-0 lg:flex lg:justify-between" sideUpsell={props.sideUpsell} overlay={true} />
                        )}
                    </div>
                </>
            )}
...
const ProductCardAbTestV3 = (props:any) => {
    ...
    return !props.useCardTemplate ? (
        <div key={props.keyName} className={`product-card !mb-[30px] lg:!mb-[45px] ${props.carousel ? 'product-card__carousel' : ''} ${props.className} ${!props.className ? 'w-3/4 md:w-1/4 pr-4 pl-4 text-center' : ''}`}>
            <a onClick={trackLink} href={props.product.handle ? `/products/${props.product.handle}` : '#'} className="rounded-[1em] product-card--img block">
                <picture className={`w-full h-full max-w-full left-0 embed-responsive before:pt-[100%] block relative rounded-[1em] ${!props.product.src ? 'bg-shimmer' : ''} bg-pink-light`}>
                    ...
                    {productHoverImage && !productHoverImage.includes('shopify/assets/no-image') && (
                        <picture className="w-full h-full max-w-full left-0 embed-responsive-item fit--cover rounded-[1em] img--hover hidden lg:block">
                            {productHoverImage && <img src={productHoverImage} className="embed-responsive-item fit--cover rounded-[1em]" alt="" loading="lazy" />}
                        </picture>
                    )}
                </picture>
            </a>
            ...
            <div className={`pt-[10px] pb-0 ${props.carousel && !props.shopArticle ? 'px-[8px] lg:px-[1rem]' : 'px-0'} ${props.quizResult ? 'lg:px-2' : props.sideUpsell ? 'lg:px-[5px]' : ''} relative grow flex flex-col product-card__content ${props.collectionTemplate ? '!px-[0rem]' : ''}`}>
                <p className={`!mb-[0px] product-title__text lg:!min-h-[25px] grow flex flex-col items-start h-100 ${props.shopArticle ? 'lg:min-h-[3.125em] lg:text-sm sm:text-lg leading-[1.25] lg:mb-[1rem!important] sm:mb-[10px!important]' : 'text-lg'} ${props.quizResult ? 'mb-0' : ''} ${props.carousel ? `${props.sustainability ? 'lg:min-h-[62.5px]' : ''} ${props.product.title.length > 40 ? 'lg:mx-0' : 'lg:mx-[0.625rem]'}` : 'px-0 lg:px-0'} ${props.quizResult ? '!min-h-0' : ''} ${props.homePage ? 'lg:min-h-[3.125em]' : ''} lg:min-h-[auto]`}>
                ...
                <div className="review-stars__number min-h-[20px] flex mb-hg">
                ...
                {props.isLaunchWL && selectedVariant && (
