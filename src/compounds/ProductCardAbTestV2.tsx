...
const WaitlistButton = (props:any) => {
    ...
    return (
        <Button onClick={handleWaitlist} buttonClass={`${props.className ?? ''} w-full text-primary font-normal text-primary border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} !h-[38px] !min-h-[38px] flex flex-row btn-sm md:text-base bg-gray-400 text-primary rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1 justify-between' : 'sm:flex-col sm:text-sm  '}`}>
...
const LaunchButton = (props: any) => {
    ...
    const data = {...props, ...{ label: `<span class="text-sm">Waitlist</span>` }};
    return (
        <Button onClick={handleLaunchWaitlist} buttonClass={`${props.className ?? ''} w-full !h-[38px] !min-h-[38px] border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} flex flex-row btn-sm md:text-base bg-gray-400 text-primary font-normal rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1 justify-between' : 'sm:flex-col sm:text-sm  '}`}>
            <Pricing store={props.store} selectedVariant={props.selectedVariant} collectionTemplate={props.collectionTemplate} props={data} hideCent={true}/>
        </Button>
    )
}
...
const AddToCartButton = (props:any) => {
    ...
    return (
        <Button onClick={onAddItem} buttonClass={`${props.className ?? ''} w-full lg:w-auto !h-[38px] !min-h-[38px] product-card-btn border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} flex flex-row btn-sm md:text-base bg-gray-400 text-primary font-bold text-primary rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1' : props.sideUpsell ? 'flex flex-col sm:text-sm lg:flex-col lg:justify-center lg:py-[5px]' : 'sm:flex-col sm:text-sm  '}`}>
    ...
    return !props.useCardTemplate ? (
        <div key={props.keyName} className={`product-card !mb-[30px] lg:!mb-[40px] ${props.carousel ? 'product-card__carousel' : ''} ${props.className} ${!props.className ? 'w-3/4 md:w-1/4 pr-4 pl-4 text-center' : ''}`}>
            ...
            <div className={`pt-[10px] lg:pt-[1rem] pb-0 ${props.carousel && !props.shopArticle ? 'px-[8px] lg:px-[1rem]' : 'px-0'} ${props.quizResult ? 'lg:px-2' : props.sideUpsell ? 'lg:px-[5px]' : ''} relative grow flex flex-col product-card__content ${props.collectionTemplate ? '!px-[0rem]' : ''}`}>
                ...
                <div className="review-stars__number min-h-[20px] flex mb-1">
