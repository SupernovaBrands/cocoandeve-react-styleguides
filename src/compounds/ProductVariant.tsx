...
const Swatch: React.FC<SwatchProp> = (props) => {
	return props.hideSwatch ? <></> : (
		<div className={`product-swatch mb-1 ${props.className}`}>
			{props.children}
			{props.shadeData && props.shadeData.map((s: any) => {
				return s.id === props.selectedSwatch ?
					(<p className={`font-size-sm w-full mt-2 mb-0 swatch-label-${s.id}`} dangerouslySetInnerHTML={{ __html: s.text }} />)
				: <></>;
			})}
		</div>
	);
	...
	return props.notes.length > 0 ? (
		<div className="notes">
			{props.notes.map((note) => (
				<p className="product-variant__description mb-1 font-size-sm">{note}</p>
			))}
        </div>
	) : <></>;
}
...
const ProductVariant: React.FC<VariantProp> = (props) => {
	return (
		<div className={`product-variant custom-radio w-full relative ${props.className}`}>
