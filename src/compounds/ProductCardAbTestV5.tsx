...
const WaitlistButton = (props:any) => {
    ...
    return (
        <Button onClick={handleWaitlist} buttonClass={`${props.className ?? ''} w-full font-bold text-body border border-primary ${props.sustainability ? '' : 'lg:border-1'} !h-[38px] !min-h-[38px] flex flex-row btn-sm text-sm text-body bg-white rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1 justify-between' : 'sm:flex-col sm:text-sm  '}`}>
...
const LaunchButton = (props: any) => {
    ...
    const data = {...props, ...{ label: `<span class="text-sm">Waitlist</span>` }};
    return (
        <Button onClick={handleLaunchWaitlist} buttonClass={`${props.className ?? ''} w-full !h-[38px] !min-h-[38px] border border-primary ${props.sustainability ? '' : 'lg:border-1'} flex flex-row btn-sm text-sm text-body bg-white font-bold rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1 justify-between' : 'sm:flex-col sm:text-sm  '}`}>
            <Pricing store={props.store} selectedVariant={props.selectedVariant} collectionTemplate={props.collectionTemplate} props={data} hideCent={true}/>
        </Button>
    )
}
...
const AddToCartButton = (props:any) => {
    ...
    return (
        <Button onClick={onAddItem} buttonClass={`${props.className ?? ''} w-full lg:w-auto !h-[38px] !min-h-[38px] product-card-btn border border-primary ${props.sustainability ? '' : 'lg:border-1'} flex flex-row btn-sm md:text-base text-sm text-body bg-white font-bold text-body rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 !px-1  mb-1' : props.sideUpsell ? 'flex flex-col sm:text-sm lg:flex-col lg:justify-center lg:py-[5px]' : 'sm:flex-col sm:text-sm  '}`}>
...
const ProductCardAbTestV3 = (props:any) => {
    ...
    return !props.useCardTemplate ? (
        <div key={props.keyName} className={`product-card !mb-[30px] lg:!mb-[22.5px] ${props.carousel ? 'product-card__carousel' : ''} ${props.className} ${!props.className ? 'w-3/4 md:w-1/4 pr-4 pl-4 text-center' : ''}`}>
