                                        ...
                                        <ol className="list-unstyled">
                                            {menu.rows.map((row) => (
                                                <li key={`menuRow-${row.title}`} className="mb-1 ">
                                                    <a href={row.handle} className={`text-body ${row.title === 'Tan Quiz' || row.title === 'SPF Quiz' ? "text-body underline decoration-[#CE8011] underline-offset-4 relative inline-block" : ''}`} aria-label={row.title}>{row.title}</a>
                                                </li>
                                            ))}
                                            {menu.title === 'Hair' && (
                                                <li className="mb-1 ">
                                                    <a href="/pages/hair-concerns-solutions" className="text-body underline decoration-[#CE8011] underline-offset-4 relative inline-block" aria-label="Hair Concerns & Solutions">Hair Concerns & Solutions</a>
                                                </li>
                                            )}
