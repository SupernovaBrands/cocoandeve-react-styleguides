...
const PostCard: React.FC<PropType> = (props) => {
	...
	return (
		<article className={`${className}`}>
			<figure className={`border border-secondary-light ${template === 'blog' || template === 'pdp' ? '' : 'post-card mb-4 lg:mb-0 bg-white h-full'} ${template === 'article' ? 'lg:mx-0 sm:-mx-g !h-auto lg:!h-full !mb-0' : ''} h-auto lg:h-full flex flex-col ${height} rounded`}>
				{data.img && (
				...
				<a href={data.handle ?? '#'} className="no-underline hover:no-underline flex flex-col flex-grow">
					<figcaption className={`${template === 'blog' || template === 'pdp' || template === 'article' ? 'p-2 flex-grow flex' : 'flex-grow'} flex flex-col ${textClass}`}>
						...
						{template === 'blog' || template === 'pdp' || template === 'article' ? (
							<span className={`btn btn-outline-primary self-start hover:no-underline leading-[1.25!important] mt-0 mb-0 flex rounded ${showSubtext ? 'lg:py-[14px] lg:px-[53px]' : 'lg:mt-auto'}`} aria-label={data.title}>Read more</span>
						) : (
