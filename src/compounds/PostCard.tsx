...
const PostCard: React.FC<PropType> = (props) => {
	...
	return (
		<article className={`${className}`}>
			<figure className={`${template === 'article' ? 'lg:mx-0 sm:-mx-g' : ''} ${template === 'blog' || template === 'pdp' ? 'border border-secondary-light' : 'post-card mb-4 lg:mb-0 bg-white h-full'} h-full flex flex-col ${height} rounded`}>
				{data.img && (
				...
				<a href={data.handle ?? '#'} className="no-underline hover:no-underline flex flex-col flex-grow">
					<figcaption className={`${template === 'blog' || template === 'pdp' ? 'p-2 flex-grow flex' : 'flex-grow'} flex flex-col ${textClass}`}>
						...
						<p className={`font-bold text-lg ${template === 'blog' ? 'blog-post__card' : ''} ${template === 'article' ? 'hover:text-primary hover:underline px-3' : 'mt-2 hover:no-underline'} leading-[1.25!important] mb-0 block text-body hover:text-body`}>{data.title}</p>
						...
						{template === 'blog' || template === 'pdp' ? (
							<span className="btn btn-outline-primary self-start hover:no-underline leading-[1.25!important] mt-auto mb-0 flex rounded" aria-label={data.title}>Read more</span>
						) : (
