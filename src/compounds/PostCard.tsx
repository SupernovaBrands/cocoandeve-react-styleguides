...
type PropType = {
	...
	figcaptionPadding?: string
	badgePadding?: string
	fontWeight?: string
}

const PostCard: React.FC<PropType> = (props) => {
	const { className, data, template, imgClass, textPrimary, textClass, height, pictureClass, showSubtext, bgColor, textColor, store, readMoreLink, figcaptionPadding, badgePadding, fontWeight } = props;
	const tagText = [];
	...
	return (
		<article className={`${className}`}>
			<figure className={`border-none border-secondary-light ${template === 'blog' || template === 'pdp' ? '' : 'post-card mb-4 lg:mb-0 bg-white h-full'} ${template === 'article' ? 'lg:mx-0 h-full !mb-0' : ''} h-auto lg:h-full flex flex-col ${height} rounded-none`}>
				{data.img && (
				...
				<a href={data.handle ?? '#'} className="no-underline hover:no-underline flex flex-col flex-grow" aria-label={ariaLabel}>
					<figcaption className={`${template === 'blog' || template === 'pdp' || template === 'article' ? 'flex-grow flex' : 'flex-grow'} ${figcaptionPadding} flex flex-col ${textClass}`}>
						<div className={`${template === 'article' || template === 'blog' ? 'badge-blog flex gap-[8px]' : ''}`}>
							{data.tags.length > 0 && (
								<>
									{data.tags.map((item: string, index: number) => (
										<PostTag store={store} widthClass={template === 'blog' ? 'min-w-[3.375em]' : ''} paddingClass={`${template === 'pdp' ? 'py-[.5em] px-[1em] mr-[4px]' : ''} ${template === 'blog' || template === 'article' ? `${badgePadding}` : ''}`} key={`article-tag-${item}-${index}`} tag={item}>{item.charAt(0).toUpperCase() + item.slice(1)}</PostTag>
									))}
								</>
							)}
						</div>
