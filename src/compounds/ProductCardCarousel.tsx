...
const Pricing = ({ props, collectionTemplate, hideCent, selectedVariant, store }) => {
    ...
    return (
        <>
            <span className={`${collectionTemplate ? 'border-x border-x-transparent' : ''} font-semibold lg:font-normal product-card-btn__text lg:w-auto flex ${props.carousel || props.collectionTemplate ? 'w-auto text-nowrap text-left py-[.8125em]' : props.sideUpsell ? 'w-full lg:w-full text-center text-sm' : 'w-full text-center lg:text-left'} `}>
                { !props.addingItem &&  parse(label ? label : DEFAULT_LABEL) }
                { props.addingItem && <span className="spinner-border spinner-border-sm text-primary lg:text-white ml-1 !w-[15px] !h-[15px]" role="status" /> }
            </span>
...
const AddToCartButton = (props:any) => {
    ...
    return (
        <Button onClick={onAddItem} buttonClass={`product-card-btn border border-[transparent] ${props.sustainability ? '' : 'lg:border-0'} flex flex-row btn-sm md:text-base btn-primary rounded-full mb-1 sm:px-0 px-0 ${props.carousel || props.collectionTemplate ? 'items-center justify-between !py-0 mb-1' : props.sideUpsell ? 'flex flex-col sm:text-sm lg:flex-col lg:justify-center lg:py-[5px]' : 'sm:flex-col sm:text-sm lg:justify-between lg:!px-g'} font-normal ${props.className ?? ''}`}>
...
const SwatchOverlay = (props:any) => {
    ...
    return (
        <>
                ...
                return resultVariant && (
                    <>
                        ...
                        {resultVariant.availableForSale && (
                            <AddToCartButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} preOrders={preOrders} comparePrice={comparePrice} price={price} selectedVariant={selectedVariant} carousel={props.carousel} addToCart={addToCart} className={`button-overlay z-[1] w-full mb-0 px-g ${props.buttonClass}`}/>
                        )}
                        {!resultVariant.availableForSale && (
                            <WaitlistButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} setWaitlistData={props.setWaitlistData} product={props.product} selectedVariant={resultVariant} comparePrice={comparePrice} price={price} carousel={props.carousel} className={`button-overlay z-[1] w-full mb-0 px-g ${props.buttonClass}`} />
                        )}
                    </>
                )
            })()}
            ...
            {!props.quizResult && (
                <>
                    <AddToCartButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} comparePrice={comparePrice} price={price} carousel={props.carousel} selectedVariant={selectedVariant} className={`btn-choose mb-1 ${props.buttonClass}`} label={labelText} addToCart={false} sideUpsell={props.sideUpsell} trackEvent={props?.trackEvent} />
                    <div className={`!w-auto px-0 swatch-overlay left-[8px] !lg:left-[0px] right-[8px] !lg:right-[0px] bottom-[35px] flex-col items-center justify-end pb-0 absolute bg-white lg:px-0 border border-primary rounded-t`}>
                        ...
                        {swatchAvailable && (
                            <AddToCartButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} label={label} preOrders={preOrders} comparePrice={comparePrice} price={price} selectedVariant={selectedVariant} carousel={props.carousel} addToCart={addToCart} className={`button-overlay z-[1] w-full mb-0 !px-g`} sideUpsell={props.sideUpsell} trackEvent={props?.trackEvent || null} />
                        )}
                        {!swatchAvailable && (
                            <WaitlistButton store={store} sustainability={props.sustainability} collectionTemplate={props.collectionTemplate} setWaitlistData={props.setWaitlistData} product={props.product} selectedVariant={selectedVariant} comparePrice={comparePrice} price={price} carousel={props.carousel} className={`button-overlay z-[1] w-full mb-0 !px-g`} sideUpsell={props.sideUpsell} />
                        )}
                    </div>
                </>
            )}
...
const ProductCard = (props:any) => {
	...
	return !props.useCardTemplate ? (
        <div key={props.keyName} className={`product-card product-card__overlay ${props.carousel ? 'product-card__carousel' : ''} ${props.className} ${!props.className ? 'w-3/4 md:w-1/4 pr-1 pl-1 lg:pr-4 lg:pr-4 text-left' : ''}`}>
            <div className="rounded-[1em] lg:rounded-[1.5em] product-card--img block mb-1">
                <picture className={`w-full h-full max-w-full left-0 embed-responsive before:pt-[100%] block relative rounded-[1em] lg:rounded-[1.5em] ${!props.product.src ? 'bg-shimmer' : ''} bg-pink-light`}>
                    <a onClick={trackLink} href={props.product.handle ? `/products/${props.product.handle}` : '#'}>
                        {productImage && <source srcSet={productImage} media="(min-width: 992px)" />}
                        {productImage && <img src={productImage} className="rounded-[1em] lg:rounded-[1.5em] bg-pink-light embed-responsive-item fit--cover !max-w-full !w-full !h-full !top-[-2.5%] !left-0 !right-auto lg:!max-h-[100%] lg:!w-full lg:!h-full lg:!max-w-full lg:!top-0 lg:!left-0 lg:!right-0" alt={product?.title} loading="lazy" />}
                        ...
                        {productHoverImage && !productHoverImage.includes('shopify/assets/no-image') && (
                            <picture className="w-full h-full max-w-full left-0 embed-responsive-item fit--cover rounded-[1em] lg:rounded-[1.5em] img--hover hidden lg:block">
                                {productHoverImage && <img src={productHoverImage} className="rounded-[1em] lg:rounded-[1.5em] bg-pink-light embed-responsive-item fit--cover !max-w-full !w-full !h-full !top-[-2.5%] !left-0 !right-auto lg:!max-h-[100%)] lg:!w-full lg:!h-full lg:!max-w-full lg:!top-0 lg:!left-0 lg:!right-0" alt="" loading="lazy" />}
                            </picture>
                        )}
                    </a>

                    <figcaption className="absolute product-card__overlay-box bottom-0 px-2 w-full">
                        <div className="w-full">
                            {!props.isLaunchWL && !props.product.swatch && selectedVariant?.availableForSale && (
                                <AddToCartButton store={store}
                                className="!px-g w-full"
                                sustainability={props.sustainability}
                                collectionTemplate={props.collectionTemplate}
                                quizResult={props.quizResult}
                                quizResultSku={props.quizResultSku}
                                preOrders={preOrders}
                            ...
                            {!props.isLaunchWL && props.product.swatch && props.product.availableForSale &&
                                ...
                                quizResultSku={props.quizResultSku}
                                preOrders={preOrders}
                                setWaitlistData={props.setWaitlistData}
                                swatch={props.product.swatch}
                                price={props.product.price}
                                comparePrice={props.product.comparePrice}
                                carousel={props.carousel}
                                product={props.product}
                            ...
                            {!props.isLaunchWL && !props.product.swatch && !selectedVariant?.availableForSale && props.product.availableForSale && (
                                ...
                                product={props.product}
                                comparePrice={props.product.comparePrice}
                                price={props.product.price} carousel={props.carousel}
                                sideUpsell={props.sideUpsell} />
                            )}
                            ...
                            {props.isLaunchWL && (
                                ...
                                className="!px-g w-full"
                                store={store}
                                launchBox={props.launchBox}
                                sustainability={props.sustainability}
                                collectionTemplate={props.collectionTemplate}
                                selectedVariant={selectedVariant}
                                setLaunchWLModal={props.setLaunchWLModal}
                                setLaunchWLModal2={props.setLaunchWLModal2}
                                setLaunchWLModal3={props.setLaunchWLModal3}
                                product={props.product}
                                comparePrice={props.product.comparePrice}
                                price={props.product.price}
                                carousel={props.carousel}
            ...
            <div className={`pt-0 pb-0 ${props.carousel && !props.shopArticle ? '' : 'px-25'} ${props.quizResult ? 'lg:px-2' : props.sideUpsell ? 'lg:px-[5px]' : 'lg:px-1'} relative grow flex flex-col product-card__content ${props.collectionTemplate ? 'px-[.5rem]' : ''}`}>
                <p className={`product-title__text text-left grow flex flex-col items-start justify-start h-100 !mb-0 ${props.shopArticle ? 'lg:min-h-[3.125em] lg:text-sm sm:text-lg leading-[1.25] lg:mb-[1rem!important] sm:mb-[10px!important]' : 'text-lg'} ${props.quizResult ? 'mb-0' : ''} ${props.carousel ? `${props.sustainability ? 'lg:min-h-[62.5px]' : ''} ${props.product.title.length > 40 ? 'lg:mx-0' : 'lg:mx-[0.625rem]'}` : 'px-0 lg:px-0'} ${props.quizResult ? '!min-h-0' : ''} ${props.homePage ? 'lg:min-h-[3.125em]' : ''} lg:min-h-[auto]`}>
                ...
                <div className="review-stars__number min-h-[20px] flex justify-start mb-0 lg:mb-1">
                    {skus.length > 0 && (<YotpoStar sustainability={props.sustainability} smSingleStar={smSingleStar} smSingleStarAllDevice={smSingleStarAllDevice} sku={skus.join(',')} productId={props.product.productId} productHandle={props.product.handle} showTotal={false} />)}
                </div>
                <span className={`${props.collectionTemplate ? 'border-x border-x-transparent' : ''} hidden lg:block w-full product-card-btn__prices lg:w-auto flex ${props.carousel || props.collectionTemplate ? 'w-auto text-left' : props.sideUpsell ? 'w-full lg:w-full text-center text-sm' : 'w-full text-center lg:text-right'}`}>
                    {selectedVariant?.compareAtPrice && (<span className="line-through mr-25 font-normal">
                        {props.hideCent && selectedVariant?.compareAtPrice ? formatMoney(Math.trunc(parseFloat(selectedVariant.compareAtPrice.amount)) * 100, false, store) : props.product.comparePrice}
                    </span>)}
                    <span className="font-semibold">{
                        props.hideCent && selectedVariant?.price ? formatMoney(Math.trunc(parseFloat(selectedVariant.price.amount)) * 100, false, store) : props.product.price}
                    </span>
                </span>
                <div className="lg:hidden w-full box-overlay relative">
                    {!props.isLaunchWL && !props.product.swatch && selectedVariant?.availableForSale && (
                        <AddToCartButton store={store}
                        className="!bg-[transparent] text-black !px-hg w-full"
                        sustainability={props.sustainability}
                        collectionTemplate={props.collectionTemplate}
                        quizResult={props.quizResult}
                        quizResultSku={props.quizResultSku}
                    ...
                    {!props.isLaunchWL && props.product.swatch && props.product.availableForSale &&
                        <SwatchOverlay
                        buttonClass="!bg-[transparent] text-black !px-hg"
                        handleShade={handleShade}
                        store={store}
                        sustainability={props.sustainability}
                        collectionTemplate={props.collectionTemplate}
