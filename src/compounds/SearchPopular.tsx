...
const toProductModel = (p: any) => {
	...
	return {
		...
		variants: {
			nodes: rawNodes.length > 0 ? rawNodes : [{ id: `fallback-${p.handle}`, availableForSale: true, price: p.price || { amount: '0' }, compareAtPrice: null, title: 'Default Title', selectedOptions: [] }],
		},
	};
};

const PopularProducts = (props: any) => {
	const { content, keywords, onClickTag, dummy, popProducts, store, trackEvent, addToCart, setWaitlistData, preOrders, generalSetting } = props;
