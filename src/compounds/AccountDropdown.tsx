...
const AccountDropdown = (props:any) => {
    ...
    return (
        <div id="account-dropdown" className={`dropdown-menu w-full ${isStickyEnabled && scrolled ? 'top-[87px]' : scrolled || !annBarEnabled ? 'top-[44px]' : 'top-[87px]'} ${timerData && timerData.notice_bar ? 'lg:top-[3.5em]' : 'lg:top-[3em]'} right-0 left-auto border-0 rounded-0 pb-0 ${isStickyEnabled && isScrollEnabled ? '-mt-[3px]' : '-mt-[1px]'} lg:mt-0 pt-0 float-none ${!onModal ? 'z-[1030] fixed lg:absolute lg:w-[330px]' : ''} ${openAccountBox ? 'block' : 'hidden'}`}>
                {activeFrame && (
