...
export default class Discounts {
    ...
    */
	getDiscountData(cart) {
		...
		if (discountCode) {
			...
			discountData.error = !discountCode.applicable && discountCode.code ? globalThis.tStrings.discount_error : '';
			...
			if (discountCode.code && global.customerEmail === '' && globalThis.tSettings.logged_in_customer_code_valid.indexOf(discountCode.code.toLowerCase()) >= 0) {
				discountData.error = globalThis.tSettings.logged_in_customer_code_error;
				...
				discountData.error = globalThis.tSettings.discode_eligible_items_only_min_spend_error.replace('$minSpend', formatMoney(parseInt(priceRule.prerequisite_subtotal_range.greater_than_or_equal_to, 10) * 100));
			}
		}

		const customCodes = globalThis.tSettings && globalThis.tSettings.custom_codes_code ? globalThis.tSettings.custom_codes_code.toUpperCase() : '';
		if (!discountData.isValid && discountData.code
            && discountData.code.toUpperCase() === customCodes && globalThis.tSettings.enable_custom_codes) {
			discountData.error = globalThis.tSettings.custom_error_codes_msg;
		}
	...
	async checkAppliedDiscounts() {
		...
		return new Promise((resolve) => {
			...
			if (discountCode) {
				...
				} else if (discountCode.code.toLowerCase() === globalThis.tSettings.customDiscountCode.code.toLowerCase()) {
	...
	*/
	isCustomCodeValid(cart, priceRule) {
		...
		const minSpend = parseFloat(globalThis.tSettings.customDiscountCode.minSpend) * 100;
				...
				if (discountedItems.length && minSpend < cart.totalBeforeShipping) {
					discountedItems.forEach(async (item, index) => {
						const attributes = [
							...
							{ key: '_discount_amount', value: globalThis.tSettings.customDiscountCode.discountAmount },
						];
						const lines = [{
							id: item.id,
							quantity: item.quantity,
