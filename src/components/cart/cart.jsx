						...
						{!loadingInit && (itemCount === 0 ? (
							...
							>
								...
								<div className="row">
									...
									<p className="col-4 mb-1 font-weight-bold text-right" data-cy="cart-subtotal-value">{Shopify.formatMoney(cart.subtotalPrice, $('#shop_currency_val').text())}</p>
									...
									{!combineDiscount && cart.discountBundleAmount > 0 && !isSwellDiscCode && (
										<>
											...
											<p className="col-4 mb-1 font-weight-bold text-right" data-cy="cart-bundledisount-value">{`-${Shopify.formatMoney(cart.discountBundleAmount, $('#shop_currency_val').text())}`}</p>
										</>
									)}
									...
									{!combineDiscount && cart.discountLine > 0 && !isSwellDiscCode && (
										<>
											...
											<p className="col-4 mb-1 font-weight-bold text-right" data-cy="cart-discount-value">{`-${Shopify.formatMoney(cart.discountLine, $('#shop_currency_val').text())}`}</p>
										</>
									)}
									...
									{combineDiscount && cart.discountCombineLine > 0 && !isSwellDiscCode && (
										<>
											<p className="col-8 mb-1 font-weight-bold" data-cy="cart-discount-label">{tStrings.cart_discount}</p>
											<p className="col-4 mb-1 font-weight-bold text-right" data-cy="cart-discount-value">{`-${Shopify.formatMoney(cart.discountCombineLine, $('#shop_currency_val').text())}`}</p>
										</>
									)}
									...
									{isSwellDiscCode && (
										<>
											...
											<p className="col-4 mb-1 font-weight-bold text-right">{`-${Shopify.formatMoney(discountData.amount, $('#shop_currency_val').text())}`}</p>
										</>
									)}
									...
									{shippingData.show && !shippingLineHide && (
										<>
											...
											<p className={`d-none d-lg-block col-4 mb-1 font-weight-bold text-right ${shippingData.amount > 0 ? '' : 'text-primary'}`} data-cy="cart-shipping-value">{shippingData.amount > 0 ? Shopify.formatMoney(shippingData.amount, $('#shop_currency_val').text()) : 'Free'}</p>
										</>
									)}
											...
											<div className="d-flex d-lg-none justify-content-between col-12">
												<p className="mb-1" data-cy="cart-shipping-label">
													...
													{(tSettings.store !== 'fr' && tSettings.store !== 'de') && shippingData.freeRate !== null && shippingData.freeRate.min_order_subtotal > 0 && (
														<span className="font-size-sm">{tSettings.cartShippingLine.shippingLine1.replace('_XX_', Shopify.formatMoney((parseFloat(shippingData.freeRate.min_order_subtotal)) * 100, $('#shop_currency_val').text())).replace('.00', '').replace(',00', '')}</span>
													)}
													{(tSettings.store === 'fr' || tSettings.store === 'de') && shippingData.freeRate !== null && shippingData.freeRate.min_order_subtotal > 0 && (
												...
												<p className={`mb-1 font-weight-bold text-right ${shippingData.amount > 0 ? '' : 'text-primary'}`} data-cy="cart-shipping-value">{shippingData.amount > 0 ? Shopify.formatMoney(shippingData.amount, $('#shop_currency_val').text()) : 'Free'}</p>
											</div>
									...
									{giftCardAmount > 0 && (
										<>
											...
											<p className="col-4 mb-1 font-weight-bold text-right">{`-${Shopify.formatMoney(giftCardAmount, $('#shop_currency_val').text())}`}</p>
										</>
									)}
								</div>
					...
					{!loadingInit && itemCount > 0 && (
						<div className="modal-footer px-g">
							<div className="row no-gutters w-100">
								...
								<strong className="col-4 font-size-lg text-right" data-cy="cart-total-value">{Shopify.formatMoney(cart.totalAmount, $('#shop_currency_val').text())}</strong>
