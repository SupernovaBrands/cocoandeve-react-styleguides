...
export const CartItem = (props:CartItemProps) => {
	...
	useEffect(() => {
		...
		if (useShopifyVariantInfo && item.merchandise?.image?.url) {
			...
			if ((store === 'my' || store === 'uk' || store === 'int' || store === 'au') && item.merchandise.product.handle === 'daily-essentials-bundle') {
				featuredImage = 'https://imagedelivery.net/ghVX8djKS3R8-n0oGeWHEA/503a39e9-27b7-4278-c850-1d015cb06000/public';
			}

			if ((store === 'my' || store === 'uk' || store === 'int' || store === 'au') && item.merchandise.product.handle === 'super-hydration-kit') {
				featuredImage = 'https://imagedelivery.net/ghVX8djKS3R8-n0oGeWHEA/764f5eec-ddf2-4ead-e1c6-88a0ab395200/public';
			}

			if ((store === 'my' || store === 'uk' || store === 'int' || store === 'au') && item.merchandise.product.handle === 'deep-condition-bundle') {
				featuredImage = 'https://imagedelivery.net/ghVX8djKS3R8-n0oGeWHEA/10045f09-b506-4fc1-c28a-b1ced8673800/public';
			}

			if ((store === 'my' || store === 'uk' || store === 'int' || store === 'au') && item.merchandise.product.handle === 'detox-nourish-set') {
