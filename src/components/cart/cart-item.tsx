...
export const CartItem = (props:CartItemProps) => {
	...
	useEffect(() => {
		...
		if (useShopifyVariantInfo && item.merchandise?.image?.url) {
			...
			if (['au'].includes(store) && (item.merchandise.product.handle === 'all-in-glow-set' || item.merchandise.product.handle === 'bronzing-self-tanner-drops') && selectedVariant[0].toLowerCase() === 'medium') {
				featuredImage = 'https://imagedelivery.net/ghVX8djKS3R8-n0oGeWHEA/e9f74cf3-1826-41a6-dde2-70b4fd315100/public';
			}

			if (['au'].includes(store) && (item.merchandise.product.handle === 'all-in-glow-set' || item.merchandise.product.handle === 'bronzing-self-tanner-drops') && selectedVariant[0].toLowerCase() === 'dark') {
