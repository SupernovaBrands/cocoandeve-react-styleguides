const PdpQuizBanner = (props) => {
    return (
        <a className={`text-body hover:text-body hover:no-underline ${props.showSpf ? 'mb-1 lg:mb-2' : 'mb-3 lg:mb-4'}  block`} href={props.quizCard.pdp_quiz_card_cta_url}>
            <div className="relative flex bg-yellow-light items-center justify-between">
                <div className="flex items-center relative">
                    <picture className="m-0 h-full aspect-[345/56] lg:aspect-[470/64]">
                        ...
                        <img alt="Tan Quiz" className="w-full" src="https://cdn.shopify.com/s/files/1/0286/1327/9779/files/tan-quiz-banner_mb2_690x.jpg?v=1776682696" loading="lazy" width={345} height={56} />
                    </picture>
                    <div className="flex absolute left-[4.75rem] top-[50%] -translate-y-[50%] lg:left-2 items-center gap-[.25rem] lg:gap-[.5rem]">
                        <strong className={`text-base !leading-[normal] lg:text-lg ${props.platform === 'os-mac' || props.platform === 'os-ios' ? 'relative top-[1px]' : ''} ${props.platform === 'os-android' ? 'relative top-[1.5px]' : ''}`}>Take the Tan Quiz</strong>
