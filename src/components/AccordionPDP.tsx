...
const AccordionPDP = (props: any) => {
	...
	const onClick = (id:any) => {
		const afterClick = () => {
			const scrollDiv = globalThis.document.getElementById(`accordionSimple`).offsetTop + 40;
		...
		<div className="border-t-0 border-b-0 md:border-t md:border-b border-gray-500 accordion w-full accordion-flush" id="accordionSimple">
			{data.map((d: Accordion, index: number) => (
				<div key={d.id} className="accordion-item border-t border-b border-gray-500">
					<div id={`accordion-${d.id}`} className={`transition-all duration-100 ease cursor-pointer flex w-full justify-between items-center ${props.openIndex === d.id ? 'pt-3 pb-3 md:pt-[1.875rem] md:pb-3' : 'py-3 md:py-[1.875rem]'} ${props.openIndex === d.id ? 'border-gray-500 accordion-opened' : ''}`} onClick={() => onClick(d.id)}>
