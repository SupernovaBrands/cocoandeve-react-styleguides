...
const AccordionPDP = (props: any) => {
	...
	return (
		<div className={`border-t-0 border-b-0 ${props?.noWrapperBorder ? '' : 'md:border-t md:border-b '} border-gray-500 accordion w-full accordion-flush`} id="accordionSimple">
			{data.map((d: Accordion, index: number) => (
				<div key={d.id} className={`accordion-item border-t border-b border-gray-500`}>
					<div id={`accordion-${d.id}`} className={`cursor-pointer flex w-full justify-between items-center ${props.openIndex === d.id ? `pt-[15px] md:pt-[30px] md:pb-[27px] ${index === 0 ?  'pb-3' : 'pb-3'}` : 'py-[15px] md:py-[30px]'} ${props.openIndex === d.id ? 'border-gray-500 accordion-opened' : ''}`} onClick={() => onClick(d.id)}>
