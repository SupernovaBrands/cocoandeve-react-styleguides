		...
		<div className={`flex items-center ${props.className}`} data-skus={props.sku}>
			...
			{props.showTotal && (
				<span className="ml-25">
					<a href={`/products/${props?.productHandle}#write-a-review`} className={`${props.smSingleStar || props.sustainability ? '' : 'text-sm'} text-body hover:text-primary underline`}>({total?.toFixed(0)})</a>
				</span>
			)}
		</div>
	) : (<div />);
