        ...
        <div id={ id ?? 'yotpoFilterForm'} className={`${className ?? ''}`}>
			...
			<div className="flex flex-wrap mt-1 w-full review__filter-form-inputs mx-0">
				<div className="w-1/2 lg:w-full pl-0 pr-0 pl-0 pr-1 lg:px-0">
					<select title="Review filter by rating" className="rounded custom-select mb-0 lg:my-1 border-0 text-sm lg:text-base h-[2.5rem] lg:h-[3.125rem]" name="scores" onChange={(e) => { onFilterChanged(e); }}>
				...
				<div className="w-1/2 lg:w-full pr-0 pl-1 lg:px-0">
					<select title="Review filter by image and video" className="rounded custom-select mb-0 lg:my-1 border-0 text-sm lg:text-base h-[2.5rem] lg:h-[3.125rem]" name="pictured" onChange={(e) => { onFilterChanged(e); }}>
				...
				{customFilter.filter((q) => !hideFilters.includes(q.slug)).map((q, i) => q.filter !== '' && (
					<div key={q.slug} className={`w-1/2 lg:w-full pr-0 lg:px-0 ${i % 2 === 0 ? 'pl-0 pr-1' : 'pl-1'}`}>
						<select title={`Review filter by ${q.slug}`} className="rounded custom-select my-1 border-0 text-sm lg:text-base h-[2.5rem] lg:h-[3.125rem]" name={q.slug} onChange={(e) => { onFilterChanged(e); }}>
