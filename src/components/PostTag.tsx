...
const PostTag: React.FC<PropType> = (props) => {
	...
	return (
		<a href={`/blogs/news/tagged/${tag}`} className={`inline-block badge no-underline hover:no-underline font-normal text-center inline-block rounded ${className}`}>{children}</a>
	)
};
