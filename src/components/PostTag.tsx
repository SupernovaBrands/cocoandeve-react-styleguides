...
const PostTag: React.FC<PropType> = (props) => {
	...
	return (
		<span onClick={(e) => handleTag(e, tag)} className={`${paddingClass} font-bold cursor-pointer inline-block badge no-underline hover:no-underline text-center mr-25 rounded ${className} ${widthClass}`}>{children}</span>
	)
};
