...
const BeautyConfidence = (props) => {
	...
	const SERVICES = [
        ...
        { id: 'stars', label: `__ratings__ stars <span class="hidden lg:inline">Customer</span> Reviews`},
	];
    ...
    return (
        <div className={props.parentClass ?? ''}>
            <div className="w-full lg:max-h-[290px] bg-gray-100 rounded-[24px] lg:rounded-[32px] text-left lg:text-center px-0 lg:py-[1.5rem] py-1 w-[75%] lg:w-full items-center [flex-flow:column] justify-center beauty-confidence--container">
                ...
                <ul className="list-unstyled flex flex-wrap items-center justify-center">
                    {SERVICES.map((list, i) => {
                        return (
                            <li key={i} className={`text-center w-1/3 mb-0 lg:flex lg:items-center lg:justify-center ${i == 1 ? 'px-0' : 'px-g sm:px-hg'}`}>
                                <i className="inline-flex flex-wrap h1 mb-g lg:mb-0">
                                ...
                                {list.id === 'stars' ? (
                                    <>
                                        {totalReviews && (
                                            <p className={`title text-sm lg:text-base mb-0 lg:inline-flex lg:ml-[1rem] ${props.className ?? ''}`}>{list.label.split('<br>').map((item) => (
