...
const NewProductModal = (props) => {
    ...
    const handleClick = (e) => {
        e.preventDefault();
        // Set cookie
        setCookie('new_product_popup', 'new_product_popup', 30);
        ...
        <div className={`new-product-modal flex w-full h-full ${bg_color} ${image_position === 'right' ? 'lg:flex-row-reverse' : 'lg:flex-row'} flex-col bg-primary-light`}>
            ...
            <div className="lg:w-1/2 w-full flex flex-col items-center justify-center p-3 lg:p-4 lg:min-w-[390px]">
                ...
                <div className="text-center w-full">
                    ...
                    <a href={cta_url} onClick={handleClick} className="btn-lg py-[15px] hover:no-underline hover:text-white w-full items-center border-2 border-primary bg-primary text-white inline-flex justify-center relative rounded-[0] mt-1 text-[16px]">
                        {cta_label}
                    </a>
                </div>
            </div>
