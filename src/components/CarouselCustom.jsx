...
const CarouselCustom = (props) => {
	...
	return (
		<div className={`paddingposition-relative ${props.useRow ? 'row' : ''} ${props.resultCard ? 'carousel--real-result' : ''}`}>
			<div
				id={`carouselLoopCentered${props.id}`}
				className={`pt-4 carousel--loop carousel--swipe carousel--centered ${props.centered ? 'carousel--centered__custom' : ''} ${!props.centered ? `carousel--centered__custom-nocenter-${props.colLgGrid}` : ''} ${props.useRow ? 'px-0' : ''} ${props.productCard || props.resultCard || props.videoCard ? '' : 'pt-2'}`}>
