...
const Accordion = (props: any) => {
	...
	return (
		<div className="border-t border-gray-500 accordion w-full accordion-flush" id="accordionSimple">
			{data.map((d: Accordion) => (
				<div key={d.id} className="accordion-item border-b border-gray-500">
					<div className={`cursor-pointer flex w-full justify-between items-center py-2 ${props.openIndex === d.id ? 'border-b border-gray-500' : ''}`} onClick={() => props.onClick(d.id)}>
						...
						{ props.openIndex === d.id && <MinusIcon className={`svg transform transition-transform size-1em`}/> }
						{ props.openIndex !== d.id && <PlusIcon className={`svg transform transition-transform size-1em`}/> }
					</div>
