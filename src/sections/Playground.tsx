    ...
    <figure className="w-full lg:w-1/4 px-g lg:px-g relative mb-g">
        <a href={store === 'ca' ? data?.button_link.replace('tan-and-spf', 'tan') : data?.button_link}>
            ...
            <figcaption className="w-2/3 md:w-full text-left lg:text-center absolute px-2 md:px-3 lg:px-g top-[50%] -translate-y-[50%] lg:transform-none lg:top-[3.125em] lg:left-0 lg:right-0">
                ...
                <span  className="btn btn-lg rounded-full btn-primary px-3 py-1 md:px-4 md:py-g border-0 border-primary no-underline hover:border-primary hover:text-white hover:no-underline">{store === 'ca' ? data.button_label.replace('Tan & SPF', 'Tan') : data.button_label}</span>
            </figcaption>
        </a>
    </figure>
);
