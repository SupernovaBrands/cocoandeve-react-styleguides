                ...
                <div className="text-center instagram-reels__video-container pl-hg lg:pl-0">
					<div className="lg:flex lg:flex-wrap lg:justify-between">
						...
						{ !props.show && (<ul className="instagram-reels__nav hide-scrollbar list-style-none mx-auto flex border-b-0 text-center justify-start px-hg flex-nowrap overflow-scroll mb-g lg:overflow-hidden lg:mx-0 lg:px-0">
							...
							{!['int', 'my'].includes(store) && (<li key={`tan`}><TabNav className={`bg-gray-400 mr-1 whitespace-nowrap rounded-full py-25 lg:py-1 px-2 lg:text-base ${activeTab === 'tan' ? 'instagram-reels__nav--active font-normal bg-primary' : 'text-gray-600'}`} title={store === 'ca' || store === 'au' ? 'Tan' : 'Tan & SPF'} active={activeTab === 'tan'} onNavChange={() => setActiveTab('tan')} ctaBgColor={generalSetting?.bfcm_cta_bg_color} ctaTextColor={generalSetting?.bfcm_cta_text_color} /></li>)}
